...
FUNCTION find_num(x: onedim; n, num: integer):integer;
Var flg: boolean;
    i: integer;
Begin 
    flg:= false;
...
BEGIN
    ...
    begin
        ...
        j:= find_num(x, n, num);
        if j <> -1 then writeln(otxt, 'В одномерном массиве есть число, кратное вашему и его номер: ', j)
        else writeln(otxt, 'Кратных чисел нет');
        
        close(otxt);
    end
    else if (not argexist) then writeln('Использование: ./main in_file.txt out_file.txt')
